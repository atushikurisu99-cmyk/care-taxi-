window.AppTimeline = (() => {
  ...
  function laneH(root) {
    const h = root ? root.clientHeight : 344;
    return Math.max(33, Math.floor((h - 62) / LANE_COUNT));
  }

  function render(root, onSelect) {
    state.root = root;
    const lh = laneH(root);
    root.style.setProperty("--lane-h", `${lh}px`);
    root.style.setProperty("--track-w", `${TRACK_W}px`);

    root.innerHTML = `
      <div class="timeline">
        <div class="timeline-left">
          <div class="timeline-left-head">${window.AppTexts.labels.driver}</div>
          ${window.AppData.drivers.map((d, i) => `
            <div class="timeline-driver" style="top:${28 + i * lh}px">${d.name}</div>
          `).join("")}
        </div>
    ...
    bindDrag();
    recalc();
    requestAnimationFrame(() => jumpToNow());
  }
  ...
  function buildGrid() {
    ...
    for (let m = START_MIN; m <= END_MIN; m += 15) {
      ...
      const isHour = m % 60 === 0;
      html += `<div class="grid-line ${isHour ? "hour" : ""}" style="left:${x}px"></div>`;
    }

    for (let i = 0; i <= LANE_COUNT; i++) {
      html += `<div class="lane-line" style="top:${i * lh}px"></div>`;
    }
  ...
  function buildJobs(onSelect) {
    ...
    window.AppData.jobs.forEach(job => {
      ...
      el.innerHTML = `
        ...
        <div class="job-state">${statusMap[job.status] || job.status}</div>
      `;
  ...
  function selectJob(id) {
    ...
    state.track.querySelectorAll(".job-band").forEach(el => {
      el.classList.toggle("is-current", el.dataset.jobId === id);
    });
  }
  ...
  function nowOffset() {
    ...
    const viewW = state.viewport.clientWidth;
    const desiredX = viewW * 0.62;
    return U.clamp(nowX - desiredX, 0, state.maxOffset);
  }

  function jumpToNow() {
    state.offset = nowOffset();
    applyOffset();
  }
  ...
  function applyOffset() {
    ...
    const viewW = state.viewport.clientWidth;
    const ratio = viewW / TRACK_W;
    const thumbW = Math.max(44, viewW * ratio);
    const thumbMax = viewW - thumbW;
    const thumbX = state.maxOffset ? (state.offset / state.maxOffset) * thumbMax : 0;
    state.thumb.style.width = `${thumbW}px`;
    state.thumb.style.left = `${thumbX}px`;
  }
  ...
  function bindDrag() {
    ...
    function move(clientX) {
      ...
      state.offset = U.clamp(state.dragStartOffset - (clientX - state.dragStartX), 0, state.maxOffset);
      applyOffset();
    }
    ...
    function end() {
      ...
      clearTimeout(state.returnTimer);
      state.returnTimer = setTimeout(animateToNow, 2600);
    }
    ...
    view.addEventListener("pointermove", e => move(e.clientX));
