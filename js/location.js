  ...
  window.CareTaxiLocation = {
      ...
      root.innerHTML = `
        <section class="location-wrap">
          ...
          <div class="location-board" id="location-board">
            ...
            <div class="location-debug-note">縦横同時スクロール禁止</div>
          </div>
        </section>
      `;
    ...
    renderBookings(bookings, vendors){
      ...
      const html = bookings.map(b => {
        ...
        if(row == null) return '';
        const left = leftForTime(b.start) + 4;
        const width = Math.max(42, leftForTime(b.end) - leftForTime(b.start) - 8);
        const top = row * cfg.rowHeight + 8;
        const tempClass = b.temporary ? 'is-temp' : '';
        const statusClass = b.unassigned ? 'is-unassigned' : '';
        const bg = colors[b.type] || '#E9F6FF';
        return `
