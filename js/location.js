...
st.canvas.innerHTML=html;var btns=st.canvas.querySelectorAll('[data-book]');for(var j=0;j<btns.length;j++){btns[j].onclick=function(){var idx=Number(this.getAttribute('data-book')),b=window.CareTaxiData.bookings[idx],v=null;for(var k=0;k<vendors.length;k++){if(vendors[k].id===b.vendorId){v=vendors[k];break;}}window.CareTaxiDetail.showBooking(b,v);};}}
  ...
  function bind(){
    ...
    st.names.addEventListener('wheel',function(e){e.preventDefault();moveBy(0,e.deltaY);}, {passive:false});
    st.names.addEventListener('touchstart',function(e){if(e.touches.length!==1)return;st.nameDown=true;st.nameSy=e.touches[0].clientY;st.nameTop=st.top;}, {passive:false});
    st.names.addEventListener('touchmove',function(e){if(!st.nameDown||e.touches.length!==1)return;e.preventDefault();var y=e.touches[0].clientY,dy=y-st.nameSy;st.top=clamp(st.nameTop-dy,0,st.maxTop);apply();}, {passive:false});
    st.names.addEventListener('touchend',function(){st.nameDown=false;}, false);
    st.names.addEventListener('mousedown',function(e){st.nameDown=true;st.nameSy=e.clientY;st.nameTop=st.top;document.body.className+=' no-select';e.preventDefault();}, false);
    window.addEventListener('mousemove',function(e){if(!st.nameDown)return;var dy=e.clientY-st.nameSy;st.top=clamp(st.nameTop-dy,0,st.maxTop);apply();}, false);
    window.addEventListener('mouseup',function(){st.nameDown=false;document.body.className=document.body.className.replace(' no-select','');}, false);
