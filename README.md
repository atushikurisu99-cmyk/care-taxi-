...
## v38 LOCATION time label fix
- v37ベース。
- 目盛範囲は 6:30〜20:00 のまま維持。
- 時間表記は 7:00〜19:00 のみ表示。
- 6:30 と 20:00 は端の目盛として使い、文字表示はしない。
- 横スクロール時の時間軸同期は維持。


## v39 iPad whole-screen lock
- v38ベース。
- UI骨格、LOCATION、DAYS、STATUS、L字、時間軸は変更しない。
- iPad Safariで画面全体が上下に動く問題を抑制。
- html/body/#appを固定し、`window.innerHeight`を`--app-h`へ反映。
- 画面全体のtouchmoveは抑制。
- LOCATIONの横スクロール、DETAIL/STATUSの内部スクロールだけ許可する下準備を追加。
